...
function renderInfoToggle() {
  return `
    <div class="summary-toggle-cell">
      <button id="infoToggle" class="summary-toggle" type="button" aria-expanded="true" aria-controls="infoDetails" aria-label="收起详细信息">
        <i class="ti ti-chevron-down account-caret" aria-hidden="true"></i>
      </button>
    </div>
  `;
...
function renderAccountInfo(queryValue) {
  ...
  return `
    <section class="portrait-section info-section">
      ...
      <div class="info-card">
        <div class="account-summary account-summary-compact summary-with-toggle">
        ...
        <div id="infoDetails" class="account-details account-details-compact">
          ${renderFieldRow("账号名", account, true)}
...
function renderDeviceInfo(queryValue) {
  ...
  return `
    <section class="portrait-section info-section">
      ${renderSectionTitle("设备信息")}
      <div class="info-card">
        <div class="device-summary summary-with-toggle">
...
function renderIpInfo(queryValue) {
  ...
  return `
    <section class="portrait-section info-section">
      ${renderSectionTitle("IP信息")}
      <div class="info-card">
        <div class="device-summary one-line summary-with-toggle">
...
function bindModalDynamicEvents() {
  ...
  if (infoToggle && infoToggle.dataset.bound !== "true") {
    ...
    infoToggle.addEventListener("click", () => {
      ...
      infoToggle.setAttribute("aria-label", expanded ? "展开详细信息" : "收起详细信息");
...
// Unified root navigation interactions.
const riskMenuTrigger = document.querySelector("#riskMenuTrigger");
const topRiskMenu = document.querySelector("#topRiskMenu");

function setTopRiskMenuOpen(open) {
  if (!riskMenuTrigger || !topRiskMenu) return;
  topRiskMenu.hidden = !open;
  riskMenuTrigger.setAttribute("aria-expanded", String(open));
}

riskMenuTrigger?.addEventListener("click", (event) => {
  event.stopPropagation();
...
topRiskMenu?.addEventListener("click", (event) => {
  ...
  showToast(`${placeholder.dataset.placeholder}入口暂未开放`);
  setTopRiskMenuOpen(false);
});

document.addEventListener("click", (event) => {
  if (!topRiskMenu?.hidden && !topRiskMenu.contains(event.target) && !riskMenuTrigger?.contains(event.target)) {
    setTopRiskMenuOpen(false);
  }
});

document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && !topRiskMenu?.hidden) setTopRiskMenuOpen(false);
});
